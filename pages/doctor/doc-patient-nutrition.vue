<template>
	...
	<view class="container">
		...
		<view @click="beginTest" style="font-weight: 400;margin-top: 100rpx; height: 90rpx;line-height: 90rpx;border-radius: 45rpx;background-color: #59A29F;color: #fff;margin-left: 60rpx;margin-right: 60rpx;text-align: center;font-family: PingFang SC;">
			给患者发送完善资料提醒消息
		</view>
		...
		<view class="info-box">
			...
			<view class="user-msg-box">
				...
				<view class="msg" style="padding-top:10rpx;">{{(infoData.illness||"")}} {{latelyData.result?(testtype==1?`${latelyData.result}`:latelyData.result):""}} {{(latelyData.result&&latelyData.result!='营养良好')?(' '+infoData.isBuy):""}}
				</view>
			</view>
		...
		methods: {
			...
			clickFuction(index) {
				if (index == 0) {
					...
					// });
					uni.navigateTo({
						url:'../patient/illness-progress?patientId='+this.uid 
					})
				}else if (index == 2){
					//测评记录
					uni.navigateTo({
